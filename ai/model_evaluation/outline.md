...
- 自动定位不应将全部日志直接输入模型，而要先按告警类型组织诊断工作流。
- 数据收集和证据压缩是系统成败关键。
- 输出应包括根因类别、证据链和可执行建议。
...
- 真实 RCA 远难于日志摘要。
- 长上下文不是通用解决方案，需要工具化检索、分步分析和证据校验。
...
- Evidence Precision：引用证据中有多少实际支持结论。
...
### 6.1 数据规模过大，直接输入上下文不可行

真实维测数据规模较大。较可靠的处理方式包括：
...
利用大模型进行维测数据自动定位的关键，是构建可审计的诊断系统：先使用观测数据、拓扑和变更记录缩小搜索空间，再由 LLM 或 Agent 生成假设、调用工具验证、输出带证据链的根因候选，并将人工确认结果持续沉淀为评测集与知识库。
