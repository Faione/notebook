...
本文从实现视角补充 [Attention Is All You Need](attention_is_all_you_need.md)。重点是 Transformer 的前向计算、计算图、核心算子，以及已有 CUDA 实现的分工。基础概念见 [机器学习基础](machine_learning_prerequisites.md) 与 [Transformer 架构](transformer_architecture.md)；原论文的运行时行为见 [Transformer 训练](transformer_training.md) 和 [Transformer 推理](transformer_inference.md)，decoder-only LLM 的算子链分别见 [LLM Prefill](decoder_only_llm_prefill.md) 与 [LLM Decode](decoder_only_llm_decode.md)。
...
> 注：`W_i^Q`、`W_i^K`、`W_i^V` 与 `W^O` 都是独立可学习矩阵。上标和下标仅标识矩阵角色，不表示运算；例如 `X_Q W_i^Q` 表示矩阵乘法。
...
训练与推理的完整说明分别见 [Transformer 训练](transformer_training.md) 与 [Transformer 推理](transformer_inference.md)。本节只保留实现视角的摘要。
...
关于 embedding、QKV 投影、attention GEMM、softmax、MLP、KV cache 读写和采样在两阶段中的完整流程图，见 [LLM Prefill](decoder_only_llm_prefill.md) 与 [LLM Decode](decoder_only_llm_decode.md)。
...
| 目标 | 建议阅读顺序 |
| --- | --- |
| 理解论文算法 | 论文导读 -> 机器学习基础 -> Transformer 架构 -> Encoder/Decoder 伪代码。 |
